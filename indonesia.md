# Indonesia investigation — baseline vs RBN at reduced epochs
...
3. `our_task_log.md` — full pipeline. **Read Step 7 carefully** — that's the BN-front-end experiment whose Indonesia behaviour we're now poking at. The 2-method headline win-rates and per-cell numbers are there.
...
2. Preprocessing: **none** only (start there; expand to other preprocessings only if Arif asks).
3. Methods: **`baseline` and `rbn`** — the two live methods.
4. Supervised training epochs: **50** (down from the project default of 200).
...
In Step 7's main 200-epoch run, Indonesia is the only dataset where RBN does *not* dominate baseline — RBN wins only 1/6 preprocessings (sgd) on Indonesia. Hypothesis: with n_train=188 the MLP head overfits well before 200 epochs, and RBN — which has more trainable params (BN γ/β scale-and-shift on 1763 features) — overfits faster than the bare MLP. Cutting to 50 epochs may flip the verdict on the cells RBN currently loses.
...
1. `results/pbn_experiment/cells/indonesia_{none,snv,msc,sg,sgd,minmax}_{baseline,rbn}.json` — the 200-epoch numbers. They are the reference comparison.
2. `results/pbn_experiment/cell_results.csv` — aggregate of the 48 main cells.
3. The runner `train_pbn_experiment.py` is idempotent and skips cells whose JSON already exists. If you want to add new cells, either change the output path or change the method-name slug (e.g. `rbn_50ep`) so the JSON sits beside, not on top of, the 200-epoch one.

## 6. Code you should reuse, not rewrite

1. `model_baseline_ann.py` — `BaselineSocAnn`. Use as-is.
2. `model_rbn_ann.py` — `RbnSocAnn`. Use as-is.
3. `train_pbn_experiment.py` — has all the helpers you need: `load_one_preprocessed_pair`, `extract_spectra_and_target`, `train_supervised_regressor(..., n_epochs)` (already takes epoch count as a parameter), `compute_metrics_dictionary`, `cell_output_paths`, `save_cell_predictions`, etc.
...
3. Comparison report: print baseline-50ep, rbn-50ep, and the existing 200-epoch numbers for the same cell side-by-side, with delta-RMSE. A 10-line print is enough — don't build a big formatter for two cells.

## 8. What to deliver back to Arif

1. The 50-epoch RMSE/R²/MBD/RPIQ for both methods on indonesia/none, plus the 200-epoch numbers for context, plus delta-RMSE.
2. A one-paragraph read of whether reducing epochs closes the RBN-vs-baseline gap, widens it, or flips it.
