...
PROBE_OUTPUT_PATH = PROJECT_ROOT / "results" / "probe_indonesia_full_batch_curve.csv"
...
def train_rbn_full_batch_with_periodic_evaluation():
    reset_all_random_seeds()
...
def main():
    ...
    metrics_per_epoch, full_batch_size = train_rbn_full_batch_with_periodic_evaluation()
    elapsed_seconds = time.time() - started_seconds

    print(
        f"\n>>> {DATASET_NAME} / {PREPROCESSING_NAME}  "
        f"(rbn, 32 hidden, FULL-batch={full_batch_size}, epochs={PROBE_EPOCHS}, "
        f"lr={PROBE_LEARNING_RATE}, weight_decay={PROBE_WEIGHT_DECAY}, seed=42)"
    )
