...
CURVE_CSV_PATH = PROJECT_ROOT / "results" / "probe_kenya_rbnd_full_batch_lr1e5_6k_l1l2_curve.csv"
OUTPUT_FIGURE_PATH = PROJECT_ROOT / "results" / "probe_kenya_rbnd_full_batch_lr1e5_6k_l1l2_test_rmse.png"

PLSR_KENYA_TEST_RMSE = 0.7567
FIXED_419_EPOCH = 419
...
def annotate_plsr_baseline(ax):
    ax.axhline(PLSR_KENYA_TEST_RMSE, color=PLSR_LINE_COLOR, linestyle="--", linewidth=1.0,
               label=f"PLSR baseline ({PLSR_KENYA_TEST_RMSE:.4f})")
...
def annotate_best_test_epoch(ax, best_epoch, best_test_rmse):
    ...
    ax.annotate(
        ...
        xytext=(best_epoch - 1400, best_test_rmse + 0.4),
...
def main():
    ...
    figure.suptitle(
        "RBND test RMSE — Kenya / none (FULL-batch=195, lr=1e-5, dropout=0.3, l1=1e-4, l2=1e-3, seed=42, 6000 ep)"
    )
