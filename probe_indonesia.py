...
def train_rbn_with_periodic_evaluation():
    ...
    return metrics_per_checkpoint


def print_curve_table(metrics_per_checkpoint):
    print(f"{'epoch':>6}  {'train RMSE':>10}  {'train R2':>9}  {'test RMSE':>10}  {'test R2':>9}")
    for row in metrics_per_checkpoint:
...
def report_best_test_checkpoint(metrics_per_checkpoint):
    best_row = min(metrics_per_checkpoint, key=lambda row: row["test_rmse"])
    print(
...
def main():
    ...
    print_curve_table(metrics_per_checkpoint)
    report_best_test_checkpoint(metrics_per_checkpoint)
